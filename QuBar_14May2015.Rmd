...
varsites<-c(varsites.t2g,varsites.t2ng,varsites.t10g,varsites.t10ng)
...
hapcounts<-c(hapCount.t2g,hapCount.t2ng,hapCount.t10g,hapCount.t10ng)
...
hapsegdata<-read.csv("hapsegData.csv",header=T)

hapMin<-ddply(hapsegdata, c("sampleSize","Populations"), summarise, MinHap = min(Haplotypes,na.rm=T))
data<-hapMin[order(hapMin$Populations),]

plot(hapMin$MinHap,hapMin$sampleSize,col=hapMin$Populations,pch=19,xlab="Minimum number of haplotypes observed",ylab="Sample size (n)")
abline(0,1)
legend("bottomright",levels(hapMin$Populations),fill=c("black","red","green","blue"),bg="white")

```

## Gamma approach

We found that the gamma approach better approximates the simulated sampling sizes in low sampling sizes (lower than 16-32 depending the population), when theta is larger (theta=10, Figure X2a) and within each theta, for the populations without growth (populations 2 and 4, Figure X2a). This method really underestimates the simulated n for larger sample sizes (Figure X2b). The probability distributions for the four populations and the 7 sampling sizes considered (2,4,8,16,32,64 and 128) are presented in the Supplementary material.


```{r Figure X2 gamma distribution plot}
library(ggplot2)
