...
row.names(miniwak)<-NULL
factor(miniwak$segSites)->miniwak$segSites

# we need to see which n has the maximum probablity for each segregating site
sumdata<-ddply(miniwak, c("Theta","segSites"), summarise, maxprob = max(prob))
# add flag
sumdata$best.n<-"best"
...
# Now we want to merge the observed segregatin sites with the segregating sites used to estimate Wakeley's formula, and that will allow us to plot the "real" n vs the best.n according Wakeley.
bymedian<-merge(miniObs,goodwak,by.x="medianOb",by.y="segSites",all.x=T)

xyplot(best.n~samplingSize|Pop,data=bymedian)
