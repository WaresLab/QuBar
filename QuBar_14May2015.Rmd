...
The approach here is identifying information that can comfortably be used as prior information to establish the posterior probability of observing polymorphism data from an *unknown* number of input individuals for a taxon. Any type of sampling information may help to set an upper limit: for example, if it is known that only about `r maxindivs` individual specimens were originally used for isolation of DNA, then the maximum number of total individuals recovered from this approach should be about `r maxindivs`. This is perhaps not exciting numerical advance in biology, but limits our prior belief nonetheless.
...
This approachs asumes that there is previous information on haplotype diversity (*H*) from the natural population of the species that are present in the barcoding sample. The 
 "haplotype diversity", *H*, defined by Nei and Tajima (1981) as $$ H = \frac{N}{N-1}(1-\sum\limits_{i=1}x_{i}^2) $$ represents the probability that sampling a new individual will result in sample of a new haplotype. N is the number of haplotypes, and *x*~i~ is the sample frequency of the i~th~ haplotype.
...
We are going to estimate the gamma probability for each of the simulated samples, using the number of haplotypes observed in the sample as the __shape/scale__ parameter and the haplotype diversity of the population where the sample comes from as the __shape/scale__ parameter. *John, we need to talk about this, weird things going on...*

## True diversity indices

*John, I'm not sure I get this properly, maybe we can discuss this at the next meeting.*
...
## Sampling theory 
*John, I may some extra text here but useful to our thinking for now*

Ewens [-@Ewens1972] developed the sampling theory of selectively neutral alleles, that based in the number of samples and the mutation parameter theta, allows to estimate the expected number of different alleles in a sample. We are going to use his approach but considering number of different haplotypes in the samples, instead of alleles.
Assuming a sample of n individuals, the mean number of haplotypes in a sample can be approximate by:

$$ E(h) = \frac{\theta}{\theta}+\frac{\theta}{\theta+1}+...+\frac{\theta}{\theta+n-1} $$

where,
h: number of different haplotypes in the sample
n: number of individuals in the sample
$\theta$: 4N~e~u

If $\theta$ is very small, the expected number of haplotypes should approximate 1, on the other hand, if $\theta$ is extremely large, the number of haplotypes should tend to n. Using this equation, we can estimate the distribution of the number of haplotypes for different sampling sizes. And we can also estimate the variance using:

$$ Var(h)= E(h)-[\frac{\theta^2}{\theta^2}+\frac{\theta^2}{(\theta+1)^2}+...+\frac{\theta^2}{(\theta+2n-1)^2}] $$

In general, the variance increases with theta for n of biological interest. Ewens [-@Ewens1972] derivations rely on the assumption that the sampling size is much lower than the population size.

## Testing the methods

We propose two/three methods that could help to estimate the relative number of individuals in a barcoding sample: 1) gamma distribution, 2) theta and segregating sites, 3) sampling theory?. To evaluate the usefulness of each method, we used a simulation approach. We simulated populations with Hudson's MS program [@Hudson 2002] using the *gap* package in R [@RCoreTeam2013]. We simulated four populations, using two/three? different mutation rates (theta= 2 and theta=10), and also considering populations with or without growth. The growth rate was modified in order to achieve a Tajima's D value that better approximates the values observed in nature for marine invertebrates. 
