...
# Figure 3
ggplot(gammaData, aes(x=samplingSize.f, y=dif,color=Theta))+ theme_bw() + geom_point(position = "jitter", aes(alpha=0.5))+ geom_hline(yintercept = 0) + scale_alpha(guide = 'none')+xlab("Simulated sampling size")+ylab("Difference in simulated vs predicted sampling size") + ggtitle("Predictive method: Gamma distribution")
```
...
#ggplot(newdata1, aes(x=samplingSize, y=difEwens,color=Theta,alpha=0.5)) + theme_bw()+geom_point()+geom_point(position = "jitter")+geom_hline(yintercept=0)+facet_grid(Theta~.)+theme(legend.key = element_blank())+ scale_alpha(guide = 'none')

factor(newdata1$samplingSize,levels=c(2,4,8,16,32,64,128)) -> newdata1$samplingSize.f

ggplot(newdata1, aes(x=samplingSize.f, y=difEwens,color=Theta,alpha=0.2)) + theme_bw()+geom_violin()+geom_point(position = "jitter")+geom_hline(yintercept=0)+facet_grid(Theta~.,scales="free")+theme(legend.key = element_blank())+ scale_alpha(guide = 'none')+xlab("Simulated sampling size")+ylab("Difference in simulated vs predicted sampling size") + ggtitle("Predictive method: Ewens's sampling theory")

#factor(newdata1$samplingSize,levels=c(2,4,8,16,32,64,128)) -> newdata1$samplingSize.f
#ggplot(newdata1, aes(x=samplingSize.f, y=difEwens,color=Theta,alpha=0.5)) + theme_bw()+geom_violin()+geom_point(position = "jitter")+geom_hline(yintercept=0)+theme(legend.key = element_blank())+ scale_alpha(guide = 'none')
```

**Figure 4.** Difference between the simulated sampling size and the sampling size for the sample predicted using Ewens's sampling theory. The abline at zero shows the ideal situation in which the estimation equals the simulated sampling size. The panels separate the three different theta: two, ten and twenty. Please note the different y-axis between panels
...
wakdata<-read.csv("WakeleyData_10Sep2015.csv",header=T)
...
ggplot(allwak, aes(x=samplingSize.f, y=dif,color=Theta)) + theme_bw()+geom_point()+geom_point(position = "jitter")+geom_hline(yintercept=0)+xlab("Simulated sampling size")+ylab("Difference in simulated vs predicted sampling size") + ggtitle("Predictive method: Wakeley's likelihood")
...
Idea brought about by extended problem-solving session with J. Drake and colleagues in the Odum School of Ecology and Department of Genetics at The University of Georgia (UGA). Our work was improved greatly by suggestions from C. Ewers-Saucedo and K. Bockrath. We thank Toby Brett for his help to code in Mathematica. This work was supported by funding from NSF (OCE-1029526) and the UGA Department of Genetics.
...
**Supplementary Figure 1.** Probability distribution of the number of individuales back calculated using the gamma approach. Each panel shows a combination of simulated sampling size and the mutation rate in the original populations.  

```{r more gamma plos,echo=FALSE,message=F,warning=FALSE}
library(ggplot2)
