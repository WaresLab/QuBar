...
for(i in thetas){
    for(j in ns){
      name <- paste(i,j)
      my.comb <- gammaData[which(minidata$Theta == i & gammaData$samplingSize == j),]
...
# Figure 3
gamma.plot <- ggplot(gammaData, aes(x=samplingSize.f, y=dif,color=Theta))+ theme_bw() + geom_point(position = "jitter", aes(alpha=0.5))+ geom_hline(yintercept = 0) + scale_alpha(guide = 'none')+xlab("Simulated sampling size")+ylab("Difference in simulated vs predicted sampling size") + ggtitle("Predictive method: Gamma distribution")
