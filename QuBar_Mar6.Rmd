...
# this one gives a funny error
ms.file <- MS(file, thetaID="Tajima", neutrality=TRUE)

# this one appears to work but give an error with some path ø?
ms.file <- MS(file,,thetaID="Tajima",neutrality=TRUE,               MSMS="-N 1000 -SAA 200 -SaA 100 -SF 1e-2")

# we can extract (when the previous thing works..) the simulated Tajima's D values
MS.get.stats(ms.file)

# NOTE!The executable file ms has to be stored in the current workspace. If you want to use the MSMS application, put the msms folder including the corresponding executable files in the current workspace.Both programs can be obtained from their websites (see references).

# SO it looks like we need to download the software to our folder... and maybe that is why the function doesn't work yet. I'll check tomorrow.
