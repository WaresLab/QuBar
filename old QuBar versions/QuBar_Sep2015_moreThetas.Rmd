...
The summary statistics of the simulated populations are presented in Table 1, the R code used to performed the simulations and estimate the summary statistics is presented in Appendix S1. As expected, the haplotype diversity, number of haplotypes and number of segregating sites are higher as $\theta$ increases. [**hmmm yes that value of D for population 6 is strange; plus we should rename populations since there are now only 3**]
...
varsites.t5ng<-popgen.t5ng@n.biallelic.sites
varsites.t10ng<-popgen.t10ng@n.biallelic.sites
varsites.t15ng<-popgen.t15ng@n.biallelic.sites
varsites.t20ng<-popgen.t20ng@n.biallelic.sites

varsites<-c(varsites.t2ng,varsites.t5ng,varsites.t10ng,varsites.t15ng,varsites.t20ng)
...
hapDiv<-c(hapDiv.t2ng,hapDiv.t5ng,hapDiv.t10ng,hapDiv.t15ng,hapDiv.t20ng)

# get haplotype counts----
length(unlist(popgen.t2ng@region.stats@haplotype.counts)/1000)->hapCount.t2ng
length(unlist(popgen.t5ng@region.stats@haplotype.counts)/1000)->hapCount.t5ng
length(unlist(popgen.t10ng@region.stats@haplotype.counts)/1000)->hapCount.t10ng
length(unlist(popgen.t15ng@region.stats@haplotype.counts)/1000)->hapCount.t15ng
length(unlist(popgen.t20ng@region.stats@haplotype.counts)/1000)->hapCount.t20ng
hapcounts<-c(hapCount.t2ng,hapCount.t5ng,hapCount.t10ng,hapCount.t15ng,hapCount.t20ng)
...
tajima<-c(Taj.t2ng,Taj.t5ng,Taj.t10ng,Taj.t15ng,Taj.t20ng)

# summary table
thetas<-c(2,5,10,15,20)
pops<-c("Population 1","Population 2","Population 3","Population 4","Population 5")
...
# Figure 3
ggplot(gammaData, aes(x=samplingSize.f, y=dif,color=Theta))+ theme_bw() + geom_point(position = "jitter", aes(alpha=0.7))+ geom_hline(yintercept = 0) + scale_alpha(guide = 'none')+xlab("Simulated sampling size")+ylab("Difference in simulated vs predicted sampling size") + ggtitle("Predictive method: Gamma distribution")
```
...
# rearrenging levels of theta for plots
newdata1$Theta <- factor(newdata1$Theta,levels=c("two","five","ten","fifteen","twenty"))
