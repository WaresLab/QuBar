...
for(i in 1:nrow(hapnum.test)){
  print(paste("working in row number",i))
    ...
    while (cdf<0.99) {
      cdfprev<-cdf
      cdf<-pgamma(x,scale=hapnum.test$hap.diversity[i],shape=hapnum.test$n.haplotypes[i]) 
      indprob<-cdf-cdfprev
      #happrob<-hapnum.test$samplingSize[i]+(x-1)
      array<-c(array,x)
      array<-c(array,cdf)
      array<-c(array,indprob)
      x=x+1
...
# save 
write.csv(all,"WakeleyData_01Sep2015.csv")
```
