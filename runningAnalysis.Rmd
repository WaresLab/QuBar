...
Below we use our observed frequency of segregating sites to estimated the sampling size according to Wakeley's formulat, for each observed theta in the natural (simulated) population of the species.
...
theta2 <-t(data.frame(do.call(rbind, ndistT2))) 
as.data.frame(theta2)->data2
data2$Theta<-"two"
data2$n<-seq(1,50,1)
...
# put data in a dataframe to handle it more easy
theta10 <-t(data.frame(do.call(rbind, ndistT10)))  
as.data.frame(theta10)->data10
data10$Theta<-"ten"
data10$n<-seq(1,50,1)
row.names(data10)<-NULL

# join dataframes and save file
rbind(data2,data10)->WakeleyData
