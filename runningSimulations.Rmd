...
hapResults[[5]]->pop5haps
hapResults[[6]]->pop6haps

# put data in a dataframe to handle it more easy
hapspop1 <-data.frame(do.call(rbind, pop1haps))
as.data.frame(hapspop1)->datahap1
datahap1$Theta<-"two"
datahap1$growth<-"yes"
datahap1$sampleSize<-c(2,4,8,16,32,64,128)
...
segResults[[5]]->pop5segsites
segResults[[6]]->pop6segsites
...
toFillhap->toFillhap5
toFillhap->toFillhap6
...
for (j in 1:7){
  ...
  names(oursize)<-NULL
  for (i in 1:100){
  toFillhap5$Haplotypes[i+count]<-oursize[i]
  toFillhap5$Population[i+count]<-"Pop5"
  }
  count+100->count
 }
for (j in 1:7){
  ...
  names(oursize)<-NULL
  for (i in 1:100){
  toFillhap6$Haplotypes[i+count]<-oursize[i]
  toFillhap6$Population[i+count]<-"Pop6"
  }
  count+100->count
 }
