...
# to get segsites for each population, we can count the number of segregating sites by counting the colummns of our sampled matrix (but first we need to do "unique")
segsitesResults<-list()
for(i in 1:length(pop)){
  populations[[i]]->popnow
  namepop<-paste("popOrigen",i,sep="")
  sapply(popnow,function(x) ncol(unique(x)))->segsites
