...
Since these calculations take quite a while (at least the haplotypes), we are going to save our results in a dataframe, the number of haplotypes and the number of segreating sites for each population.
...
# we need to define a maximum n, above 70 this behaves funny???? 
maxn=200
...
ndist<-list()
for (z in 1:nrow(segdata)){
  name<-paste("seg",z,sep="")
  obsvdk<-segdata$segSites[z]
  a <- c(1:maxn)
  b <- c(1:maxk)
